...
if [ -z "$VM_FOLDER" ]; then
  govc import.ova "$file_path" -folder="$VM_FOLDER" -name=ha-proxy-latest-ova
  ...
  govc import.ova -folder="$VM_FOLDER" "$file_path" -folder="$VM_FOLDER" -name=ha-proxy-latest-ova
  govc vm.clone -vm "$file_path" -snapshot $(govc snapshot.tree -vm "$file_path" -C) -folder="$VM_FOLDER" ha-proxy-latest
fi